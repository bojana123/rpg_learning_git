...
class DirectoryIterator(Iterator):
    ...
    def _decode_experiment_dir(self, dir_subpath):
        ...
        # Try load labels
        try:
            ground_truth = np.loadtxt(labels_filename, delimiter=',', usecols=0)
            ground_truth = ground_truth/180
            limit = 80/180
            
         # load velocities               
            velocities = np.loadtxt(labels_filename, delimiter=',', usecols=3)
            ...
            for frame_number, fname in enumerate(sorted_files):
                ...
                for extension in self.formats:
                    if fname.lower().endswith('.' + extension):
                        is_valid = True
                        break
                    #printing bar histogram of steering angles
                if ((is_valid) and (velocities[frame_number] >= 20) and (ground_truth[frame_number] <= limit)):
